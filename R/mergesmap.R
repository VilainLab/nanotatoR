...
#' @examples
#' mergingSMAP_Dual(
#' path = "Y:/Hayks_Materials/BNG/Projects/VAP_SVmerge_solo_TXTs/",
#' pattern = "*.txt", outMode = c("dataframe","Text"), 
#' outpath = "Y:/Hayks_Materials/BNG/Projects/VAP_SVmerge_solo_TXTs/Merged")
#' @export
mergingSMAP_Dual <- function(
path = "Y:/Hayks_Materials/BNG/Projects/VAP_SVmerge_solo_TXTs/",
pattern = "*.txt", outMode = c("dataframe","Text"), outpath = "Y:/Hayks_Materials/BNG/Projects/VAP_SVmerge_solo_TXTs/Merged")
{
...
for (ii in seq_along((l))){
    ...
    stop("column names doesnot Match")
        ...
        Found_in_DLE_molecules <- as.character(
            c(rep ("-", times = nrow(datfinal)))
            )
        Fail_assembly_chimeric_score_DLE <- as.character(
            c(rep ("-", times = nrow(datfinal)))
            )
        Method <- as.character(
        ...
        Fail_assembly_chimeric_score_DLE = Fail_assembly_chimeric_score_DLE, 
        ...
        Found_in_DLE_self_molecules = Found_in_DLE_molecules , 
        Method = Method
        )

        if (outMode == "Text"){
            len <- length(l)
            fileOut <- paste0("Dual_1:",len,".txt")
            write.table(datFinal_Dual, file.path(
			    outpath = outpath, fileOut), 
				row.names = FALSE)
        }
...
#' @return Text files containg merged smaps from different samples
#' @examples
#' mergedSmap <- mergingSMAP_DLE (
#' path = "Y:/Hayks_Materials/BNG/Projects/VAP_DLE1_solo_SMAPs/",
...
mergingSMAP_DLE <- function(
path = "Y:/Hayks_Materials/BNG/Projects/VAP_DLE1_solo_SMAPs/",
pattern = "*_DLE1_*",
outMode = c("Text", "dataframe"),
outpath = "Y:/Hayks_Materials/BNG/Projects/VAP_DLE1_solo_SMAPs/Merged")
{
...
for (ii in seq_along((l)))
{
    ...
    #### print(dim(datfinal))
        ...
        Method <- as.character(c(rep ("DLE", times = nrow(datfinal))))
		...
		Found_in_DLE_self_molecules = as.character(
        ...
        if (outMode == "Text"){
            len <- length(l)
            fileOut <- paste0("DLE_1:",len,".txt")
            write.table(datFinal_DLE, file.path(outpath = outpath, fileOut), 
			    row.names = FALSE
				)
        }
        else if (outMode == "dataframe"){
...
#'    mergeKey = "Y:/Hayks_Materials/BNG/Projects/MergeKey.csv",
#'    outMode = c("dataframe"),
#'   outpath = "Y:/Hayks_Materials/BNG/Projects/VAP_DLE1_solo_SMAPs/Merged")
#' @import hash
#' @import tidyverse
...
#' @param Dual_path    character. Path for the Dual labelled cmap
#' @param Dual_pattern    character. pattern of the dual files.
#' @param Dual_path    character. Path for the Dual labelled cmap
#' @param Dual_pattern    character. pattern of the dual files.
...
if(labelType == "Both"){
datDual <- mergingSMAP_Dual(path = Dual_path, pattern = Dual_pattern, outMode = "dataframe")
datDLE <- mergingSMAP_DLE(path = DLE_path, pattern = DLE_pattern, outMode = "dataframe")
'datFinal <- FamilyInfoPrep(Samplecodes, mergeKey, outMode = "dataframe")
sampleIds <- as.character(unique(datFinal$SampleID))'
datfinal <- rbind(datDual,datDLE)
}else if(labelType == "DLE"){
datDLE <- mergingSMAP_DLE(path = DLE_path, pattern = DLE_pattern, outMode = "dataframe")
datfinal <- datDLE
}else if(labelType == "Dual"){
datDual <- mergingSMAP_Dual(path = Dual_path, pattern = Dual_pattern, outMode = "dataframe")
datfinal <- datDual
...
for (n in seq_along(sampleIds)){
    ...
    dataFinal_temp$nanoID <- rep(datFinal$NID[n], times = length(dataFinal_temp$SampleID))
    dataFinal_temp$ProjectID <- rep(datFinal$ProjectID[n], times = length(dataFinal_temp$SampleID))
 ...
 #filename <- "UDN_DSD_merged_06232019.txt"
###writing the file
write.table(dataFinal, file.path(outpath, filename), row.names =FALSE, sep ="\t")
}
