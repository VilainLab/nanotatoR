...
#' returnMethod="dataFrame", perc_similarity_parents = 0.9,
#' mergedKeyoutpath = "Y:/Hayks_Materials/BNG/Projects/", 
#' mergedKeyFname = "mergedKeynanotatoR.csv ")
#' @importFrom stats na.omit 
#' @import hash
#' @export
internalFrequencyTrio_Dyad <- function(mergedFiles, smappath , smap , 
    buildSVInternalDB=FALSE, smapdata, input_fmt=c("Text","dataFrame"), 
    path, pattern,  win_indel=10000, win_inv_trans=50000, 
    perc_similarity=0.5, indelconf=0.1, invconf=0.01, fname,
    limsize=1000, win_indel_parents=5000,win_inv_trans_parents=40000, 
    transconf=0.1, dbOutput =c("dataframe","text"),
    returnMethod=c("Text","dataFrame"), perc_similarity_parents = 0.9,
    indexfile = "X:/Hayks_Materials/BNG/Projects/Sample_NIDKeys.csv",
    Dual_path = "X:/Hayks_Materials/BNG/Projects/VAP_SVmerge_solo_TXTs/",
        ...
        for (nn in seq_along(rf))
        {
            ...
            if ((variantType2[nn] == "deletion" | 
                ...
                if (nrow(dat2) > 1)
                {
                    ...
                    dat2MomEqual <- subset(dat2,(((RefStartPos ==rf[nn]) 
                            & (RefEndPos==re[nn])) 
                    ...
                    dat2DadEqual <- subset(dat2,(((RefStartPos ==rf[nn]) & 
                                    (RefEndPos==re[nn])) & 
                                    (perc_ref_query >= perc_similarity_parents 
                                    & perc_query_ref >= perc_similarity_parents) 
                                    & (as.character(Type) == variantType2[nn]) 
                    ...
                    dat2MomRange <- subset(dat2,(((
                                        RefStartPos >= rf_wb_int_parents[nn]) 
                                        & (RefEndPos <= re_fb_int_parents[nn]))
                                        & (perc_ref_query >= perc_similarity_parents 
                                        & perc_query_ref >= perc_similarity_parents) 
                                        & (as.character(Type) == variantType2[nn])
                                        & (as.character(FamilyID) == svfamid)
                    ...
                    dat2UnfilteredFrequency <- subset(
                    ...
                    if (is.null(dat2Frequency)){
                        ...
                        dat2Heterozygotes <- data.frame(
                                dat2Frequency 
                                %>% mutate(
                                Heterozygo = Zygosity == "heterozygous"
                                ) 
                        %>% group_by(nanoID) 
                ...
                {   
                    ...
                    dat2$FamilyID <- sapply(stt12, function(x) x[1])
                    dat2$RelationID <- sapply(stt12, function(x) x[2])
                    dat2MomEqual_duplication <- subset(dat2,((
					        (RefStartPos ==rf[nn]) & (RefEndPos==re[nn]))  
							& (perc_ref_query >= perc_similarity_parents 
                            & perc_query_ref >= perc_similarity_parents)
                            & (as.character(Type) == variantType2[nn])
                            & (as.character(FamilyID) == svfamid)
                    ...
                    dat2DadEqual_duplication <- subset(dat2,(((
                            RefStartPos ==rf[nn]) & (RefEndPos==re[nn]))
                            & (perc_ref_query >= perc_similarity_parents 
                            & perc_query_ref >= perc_similarity_parents)							
                            & (as.character(Type) == variantType2[nn])
                    ...
                    dat2MomRange_duplication <- subset(dat2,(((
                            RefStartPos >= rf_wb_ind_parents[nn])
                            & (perc_ref_query >= perc_similarity_parents 
                            & perc_query_ref >= perc_similarity_parents)							
                            & (RefEndPos <= re_wf_ind_parents[nn]))
                            & (as.character(Type) == variantType2[nn])
                            & (as.character(FamilyID) == svfamid)
                            & (patID == 1 & RelationID == 2)
                            & ((as.character(Type) == "duplication") 
                    ...
                    dat2DadRange_duplication <- subset(dat2,((( 
                            RefStartPos >= rf_wb_ind_parents[nn]) 
							& (perc_ref_query >= perc_similarity_parents 
                            & perc_query_ref >= perc_similarity_parents)
                            & (RefEndPos <= re_wf_ind_parents[nn])) 
                    ...
                    dat2MomEqual_insertion <- subset(dat2,(
                            ((RefStartPos ==rf[nn]) 
                            & (RefEndPos==re[nn])) 
                            & (as.character(Type) == variantType2[nn])
                            & (perc_ref_query >= perc_similarity_parents 
                    ...
                    dat2DadEqual_insertion <- subset(dat2,(
                            ((RefStartPos ==rf[nn]) & (RefEndPos==re[nn])) 
                            & (as.character(Type) == variantType2[nn])
                            & (perc_ref_query >= perc_similarity_parents 
                            & perc_query_ref >= perc_similarity_parents) 
                    ...
                    dat2MomRange_insertion <- subset(dat2,(((
                        RefStartPos >= rf_wb_ind_parents[nn]) 
                        & (RefEndPos <= re_wf_ind_parents[nn])) 
                        & (perc_ref_query >= perc_similarity_parents 
                        & perc_query_ref >= perc_similarity_parents) 
                    ...
                    dat2DadRange_insertion <- subset(dat2,(((
                        RefStartPos >= rf_wb_ind_parents[nn]) 
                        & (RefEndPos <= re_wf_ind_parents[nn]))  
                        & (perc_ref_query >= perc_similarity_parents 
                        & perc_query_ref >= perc_similarity_parents) 
                    ...
                    dat2FrequencyDuplication <- subset(dat2,(((
                            as.character(Type) == "duplication") 
                            | (as.character(Type) == "duplication_split") 
                            | (as.character(Type) == "duplication_inverted"))
							& (perc_ref_query >= perc_similarity 
                            ...
                            | (Type1 == "duplication_split" 
                            & Type2 == "duplication_split") 
                            | (Type1 == "duplication_split" & is.na(Type2)) 
                            | (is.na(Type1) & Type2 =="duplication_split") 
                            | (Type1 == "duplication_inverted" 
                            & Type2 == "duplication_inverted") 
                            | (Type1 == "duplication_inverted" 
                            & is.na(Type2)) 
                            | (is.na(Type1) 
                            & Type2 =="duplication_inverted")
                    ...
                    datf <- rbind(datf, data1)
                        ...
                        conf <- dat2$Confidence
                        #Found_in_DLE_self_molecules <- dat2$Found_in_DLE_self_molecules
                        #Fail_assembly_chimeric_score_DLE_DLE <- dat2$Fail_assembly_chimeric_score_DLE_DLE
                        BSPQI_status_DB <- 
                        as.character(dat2$Found_in_self_BSPQI_molecules)
                        BSSSI_status_DB <- 
                        as.character(dat2$Found_in_self_BSSSI_molecules)
