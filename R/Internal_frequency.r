...
internalFrequency <- function(mergedFiles, smappath, smapName,
        ...
        if (nrow(dat2) > 1) {
          ...
          for (ll in seq_len(nrow(dat2))) 
          {
            ...
            if (perc >= perc_similarity & identical(type[ll], variantType2[nn]) &
              ...
              ((BSPQI_status_DB[ll] == "yes" & BSSSI_status_DB[ll] == "yes") 
			  | (BSPQI_status_DB[ll] =="no" & BSSSI_status_DB[ll] == "yes") 
			  | (BSPQI_status_DB[ll] == "yes" & BSSSI_status_DB[ll] == "no")
			  | (BSPQI_status_DB[ll] == "yes" & BSSSI_status_DB[ll] == "-") 
			  | (BSPQI_status_DB[ll] == "-" & BSSSI_status_DB[ll] == "yes"))) {
            ...
            } else if (perc >= perc_similarity & identical(
              type[ll],
              variantType2[nn]
          ...
          if ((length(svfamid3) >= 1) & length(countfre) >= 1) {
            ...
            if (nrow(dat_temp) > 0) {
              ...
              for (u in seq_len(length(usvfamid3))) 
              {
          ...
          data1 <- data.frame(
            dat1[nn, ], Internal_Freq_Perc_Filtered = as.numeric(format(((countfre1 / (usamp -
          ...
          print(paste("ii:", ii, "svfamid:", svfamid, sep = ""))
          famno <- as.numeric(unname(hash::values(ha, keys = svfamid)))
          data1 <- data.frame(
            dat1[nn, ], Internal_Freq_Perc_Filtered = as.numeric(format(((countfre / (usamp -
          ...
          # dgv_match=FALSE
          data1 <- data.frame(
            dat1[nn, ], Internal_Freq_Perc_Filtered = "-",
      ...
      | variantType2[nn] == "duplication_inverted" |
        ...
        if (nrow(dat2) > 1) {
          ...
          for (ll in seq_len(nrow(dat2)))
          {
            ...
            } else if (identical(chrom2[ll], chromo2[nn]) & identical(
              type[ll],
              variantType2[nn]
            ) & !(identical(svfamid1, svfamid)) &
              ((BSPQI_status_DB[ll] == "yes" & BSSSI_status_DB[ll] ==
          ...
          data1 <- data.frame(
            dat1[nn, ], Internal_Freq_Perc_Filtered = "-", Internal_Freq_Perc_Unfiltered = "-",
  ...
  if (returnMethod_Internal == "Text") {
    ...
    write.table(
      dataFinal, file.path(smappath, fname), sep = "\t",
      row.names = FALSE
    )
  }
