  ...
  function(method = c("Single", "Multiple", "Text"),
    ...
    if (length(terms) >= thresh) {
      ...
      Genes <- c(
        ...
        as.character(g_1$geneName),
        as.character(g_o$omimGenes),
        as.character(g_o_1$omimGenes),
        as.character(g_g$gtrGenes),
        as.character(g_g_1$gtrGenes),
        as.character(g_c$clinvarGenes),
        as.character(g_c_1$clinvarGenes)
      )
      Terms <-
        c(
          as.character(g$Final_terms),
          as.character(g_1$Final_terms),
...
gene_extraction <- function(terms) {
  ...
  if (length(terms) > 1) {
    #checking for terms greater than 1
    for (ll in 1:length(terms)) {
      ##Searching Entrez with the terms
      b <- entrez_search(db = "gene",
                         term = terms[ll],
                         retmax = 99999999)
...
clinvar_gene <- function(terms) {
  ...
  if (length(terms) > 1) {
    for (ll in 1:length(terms)) {
      ...
      final_clinvar <-
        unique(c(
          as.character(clinvargeneID_word),
      ...
      if (length(final_clinvar) > 0) {
        ...
        terms_list <-
          as.character(rep(paste(terms[ll], "_ClinVarGene", sep = ""),
                           length(gn_4)))
