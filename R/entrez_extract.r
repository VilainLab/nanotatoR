...
>>>>>>> e89c8fe038e1adb7ae6f0ca0588a1082c0502c6b
...
#' @import biomaRt
#' @export
gene_list_generation<-function(method=c("Single","Multiple","Text"), termPath, 
  ...
  stop("Method of Input Incorrect!!!")
  }
  ...
  ##Collating output from all the datasets
  Genes<-c(as.character(g$geneName),as.character(g_g$gtrGenes),as.character(g_o$omimGenes),as.character(g_c$clinvarGenes))
  Terms<-c(as.character(g$Final_terms),as.character(g_g$Final_terms_GTR),as.character(g_o$Final_terms_OMIM),as.character(g_c$Final_terms_Clinvar))
  ##Writing genes and terms in a dataset
  dat<-data.frame(Genes,Terms)
  }
  ##We write out all the genes and terms from the different databases into a$description
  ##single dataframe
  gene1<-c()
  term1<-c()
  ...
  write.table(dat_Final,file.path(outpath,fileName),col.names=c("Genes","Terms"),row.names=FALSE,sep="\t")
  }
...
gene_extraction<-function(terms){
  ...
  for(ll in 1:length(terms)){
    ...
    if(length(geneID)>0){
      ...
      terms_list<-as.character(rep(paste(terms[ll],"_Gene",sep=""),length(gn_1)))
      Final_terms<-c(Final_terms,terms_list)
    }
    else{next}      
  }
  } else if (length(terms)==1){ #checking for terms equal to 1
...
omim_gene<-function(terms){
  ...
  for(ll in 1:length(terms)){
    ...
    if(length(final_omim)>0){
      ...
      terms_list<-as.character(rep(paste(terms[ll],"_OMIMGene",sep=""),length(gn_2)))
    ...
    c<-entrez_search(db="omim",term=paste(terms,"[WORD]",sep=""),retmax=99999999)
    d<-entrez_search(db="omim",term=paste(terms,"[DIS]",sep=""),
	      retmax=99999999)
    omimgeneID_word<-c$ids
    omimgeneID_dis<-d$ids
    final_omim<-unique(c(as.character(omimgeneID_word),as.character(omimgeneID_dis)))
    ...
    if(length(final_omim)>0){
      ...
      gene2 = getBM(attributes = c("entrezgene", "ensembl_gene_id", "hgnc_symbol"), 
...
gtr_gene<-function(terms){
  ...
  for(ll in 1:length(terms)){##if term length greater than 1
    ...
    if(length(gtrgeneID)>0){
      ...
      gene3 = getBM(attributes = c("entrezgene", "ensembl_gene_id", "hgnc_symbol"), 
      ...
      terms_list<-as.character(rep(paste(terms[ll],"_GTRGene",sep=""),length(gn_3)))
...
clinvar_gene<-function(terms){
  ...
  for(ll in 1:length(terms)){
    ...
    f<-entrez_search(db="clinvar",term=paste(terms[ll],"[WORD]",sep=""),retmax=99999999)
    g<-entrez_search(db="clinvar",term=paste(terms[ll],"[DIS]",sep=""),retmax=99999999)
    ...
    final_clinvar<-unique(c(as.character(clinvargeneID_word),as.character(clinvargeneID_dis),
    ...
    if(length(final_clinvar)>0){
      ...
      terms_list<-as.character(rep(paste(terms[ll],"_ClinVarGene",sep=""),
	            length(gn_4)))
   ...
   f<-entrez_search(db="clinvar",term=paste(terms,"[WORD]",sep=""),
    ...
    if(length(final_clinvar)>0){
      ...
      gene4 = getBM(attributes = c("entrezgene", "ensembl_gene_id", "hgnc_symbol"), 
                    filters = "entrezgene", values = final_clinvar, mart = ensembl)
      gn4<-as.character(unique(gene4$hgnc_symbol))
      gn_4<-gn4[gn4 != ""]
      clinvarGenes<-c(clinvarGenes,as.character(gn_4))
      terms_list<-as.character(rep(paste(terms[ll],"_ClinVarGene",sep=""),length(gn_4)))
      Final_terms_Clinvar<-c(Final_terms_Clinvar,terms_list)
    }
	else{
	next
...
#'		downloadClinvar = TRUE, clinsig = "Pathogenic/Likely pathogenic",
