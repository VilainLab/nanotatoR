...
run_bionano_filter_duo <- function(input_fmt_geneList = c("Text", "dataFrame"),
  ...
  'list_of_datasets <- list(
    ...
    "all_PG_OV" = datovrlapPG, "all_PG_Non_OV_UP" = datnonovrlapUPPG,"all_PG_Non_OV_DN" = datnonovrlapDNPG
      ...
      if (outputType == "Excel"){
            ...
            "all_PG_OV" = datovrlapPG, "all" = data)
		    fname <- paste(outputFilename, ".xlsx", sep = "")
            write.xlsx(list_of_datasets, file = file.path(outpath, fname), keepNA = TRUE)
	    } else if (outputType == "csv"){
	        write.csv(dat10, file.path(directoryName, paste(prefix,"indel_dup_notshared.csv",sep = ""), row.names = FALSE))
	    	...
	    	write.csv(dat6, file.path(directoryName, paste(prefix,"_mismatch.csv",sep = ""), row.names = FALSE))
	    ...
	    if (outputType == "Excel"){
	        ...
	        write.csv(dat10, file.path(directoryName, paste(prefix,"_indel_dup_notshared.csv",sep = ""), row.names = FALSE))
	    	write.csv(dat8, file.path(directoryName, paste(prefix,"_inv.csv",sep = ""), row.names = FALSE))
	    	write.csv(dat7, file.path(directoryName, paste(prefix,"_trans.csv",sep = ""), row.names = FALSE))
