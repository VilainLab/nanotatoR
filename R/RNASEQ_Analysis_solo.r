...
#' @param smappath  character. Path to the SV dataset.
...
SmapRNAseqquery_solo<-function(input_fmt_SV=c("Text","dataFrame"),smapdata,smappath,
                          input_fmt_RNASeq=c("Text","dataFrame"),
                          RNASeqData,RNASeqPATH,outputfmt=c("Text","datFrame"),
                          pattern_Proband = NA, EnzymeType = c("Dual", "DLE")){
    ...
    for(nn in 1:length(nearestDNGenes))
    {
        ...
        if(length(grep(";",nearestDNGenes[nn]))>=1){
            ...
            dataFinal<-data.frame(smapdata, 
                Proband_OverlapGeneExpression_TPM = dataOverLap[,2],
                Proband_Upstream_nonOverlapGeneExpression_TPM = datanonOverLapUP[,2],
                Proband_Downstream_nonOverlapGeneExpression_TPM = datanonOverLapDN[,2])
    return(dataFinal)
