...
nanotatoR_main <- function(smap, bed, inputfmtBed = c("BED", "BNBED"),
  ...
  datDGV <- DGV_extraction(
    hgpath = hgpath, smap_data = datcompSmap, input_fmt_DGV = c("dataFrame"),
    win_indel_DGV, win_inv_trans_DGV, perc_similarity_DGV,
    returnMethod_DGV = c("dataFrame")
  )
  if (buildSVInternalDB == FALSE) {
    datInf <- internalFrequency(
      mergedFiles = mergedFiles, buildSVInternalDB = FALSE, smapdata = datDGV, input_fmt_INF = "dataFrame", win_indel_INF, win_inv_trans_INF, smapName = smapName,
  ...
  run_bionano_filter(
    input_fmt_geneList = "dataFrame", input_fmt_svMap = "dataFrame",
