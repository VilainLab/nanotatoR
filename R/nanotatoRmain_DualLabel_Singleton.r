#' Annotation of Bionano singleton samples, Dual enzyme
#'
...
#' type of SVs.
#' @return csv file containing the annotated SV for each SV type, if 
#' outputType is chosen as csv.
...
#' @export
nanotatoR_main_Dual_Singleton <-function(
	...
	print("GeneListGenerator Function !!!!")
	start_time <- Sys.time()
	...
	end_time <- Sys.time()
	print("GeneListGenerator Function Time:")
    print(end_time-start_time)
	print("Bed Comparison Function!!!!!")
	start_time <- Sys.time()
	...
	end_time <- Sys.time()
	print("Bed Comparison Function Time:")
    print(end_time-start_time)
	print("DGV generation Function !!!!")
	start_time <- Sys.time()
	datDGV <- tryCatch(
	    DGV_extraction(
		    hgpath = hgpath, 
	...
	end_time <- Sys.time()
	print("DGV generation Function Time:")
    print(end_time-start_time)
	dim(datDGV)
	print("Internal Frequency Function !!!!")
	start_time <- Sys.time()
	if(buildSVInternalDB==FALSE){
	datInf <- tryCatch(internalFrequency_singleton_dual(
    ...
    datInf <- tryCatch(
	        internalFrequency_singleton_dual(
     ...
     end_time <- Sys.time()
     print("Internal Frequency Function Time:")
     print(end_time-start_time)
	 dim(datInf)
	 print("Bionano Internal Frequency Function !!!")
	start_time <- Sys.time()
if(buildBNInternalDB==FALSE){
    ...
    end_time <- Sys.time()
	print("Bionano Internal Frequency Function Time:")
    print(end_time-start_time)
	dim(datchort)
start_time <- Sys.time()
print("Decipher Frequency Function !!!")
	start_time <- Sys.time()
...
print("RNASeq Function !!!")
    ...
    datRNASeq <- tryCatch(
		...
		smappath = smappath, 
		input_fmt_RNASeq = "dataFrame", 
		RNASeqData = RNASeqData, EnzymeType = "Dual",
		outputfmt = "datFrame",
    ...
    datRNASeq <- tryCatch(
        SmapRNAseqquery_singleton(
		input_fmt_SV = "dataFrame", 
		smapdata = datdecipher, 
		smappath = smappath, 
		input_fmt_RNASeq = "Text", EnzymeType = "Dual",
...
print("Bionano Filter Function !!!")
tryCatch( 
run_bionano_filter_singleton_Dual(
