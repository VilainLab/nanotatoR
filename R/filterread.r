...
run_bionano_filter_Trio_Dual <- function(input_fmt_geneList = c("Text", "dataFrame"),
                               input_fmt_svMap = c("Text", "dataFrame"),
                               SVFile = NULL, svData, dat_geneList, fileName, outpath,
...
"all_PG_OV" = datovrlapPG, "all_PG_Non_OV_UP" = datnonovrlapUPPG,"all_PG_Non_OV_DN" = datnonovrlapDNPG
    ...
    if (outputType == "Excel"){
		...
		write.csv(datOvrLap, file.path(directoryName, paste(prefix,"_all_PG_OV.csv",sep = ""), row.names = FALSE))
