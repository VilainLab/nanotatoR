...
#' @import hash
#' @export
internalFrequency_singleton_dual <- function(mergedFiles, smappath , smap , 
    ...
    indexfile,EnzymeType = c("Dual", "DLE"))
    {
    #library(hash)
        ...
        for (nn in seq_along(rf))
        {
            ...
            if ((variantType2[nn] == "deletion" | 
                ...
                if (nrow(dat2) > 1)
                {
                    ...
                    'dat2MomEqual <- subset(dat2,(((RefStartPos ==rf[nn]) & (RefEndPos==re[nn])) & 
                            (perc >= perc_similarity) & (as.character(Type) == variantType2[nn])
                    ...
                    dat2MomRange <- subset(dat2,((((RefStartPos <= rf[nn] & 
                        ...
                        | (RefEndPos <= re[nn] & RefEndPos >= re_wf_ind_parents[nn]))) 
                        & (perc >= perc_similarity) 
                        & (as.character(Type) == variantType2[nn])
                            & (as.character(FamilyID) == svfamid)
                            & (patID == 1 & RelationID == 2)))
                    dat2DadRange <- subset(dat2,((((RefStartPos <= rf[nn] & 
                        RefStartPos >= rf_wb_ind_parents[nn]) | 
                        (RefStartPos >= rf[nn] & RefStartPos <= rf_wb_ind_parents[nn])) 
                        & ((RefEndPos >= re[nn] & RefStartPos <= re_wf_ind_parents[nn]) 
                        | (RefEndPos <= re[nn] & RefEndPos >= re_wf_ind_parents[nn]))) 
                    ...
                    ##Zygosity Parent
                        ...
                        data1 <- data.frame(dat1[nn, ], 
                            Internal_Freq_Perc_Filtered = as.numeric(format(round((filtFreq/(2*(usamp - famnumb))) * 100,digits=3) , scientific = FALSE)),
                            Internal_Freq_Perc_Unfiltered = as.numeric(format(round((UnfilteredFrequency/(2*(usamp - famnumb))) * 100,digits=3), scientific = FALSE)),
                ...
                {   
                    ...
                    ## Calculating Internal Frequency
                    if (nrow(dat2) > 1){ 
                    dat2$perc_query_ref <- (size1/dat2$Size)
                    dat2$perc_ref_query <- (dat2$Size/size1)
                    stt12 <- str_split(as.character(dat2$nanoID), pattern = "\\.")
                    famid <- c(); relnid <- c()
                    dat2$FamilyID <- sapply(stt12, function(x) x[1])
                    dat2$RelationID <- sapply(stt12, function(x) x[2])
                    'dat2MomEqual <- subset(dat2,(((RefStartPos ==rf[nn]) & (RefEndPos==re[nn])) & 
                            ((perc_ref_query >= perc_similarity & perc_query_ref >= perc_similarity)) & (as.character(Type) == variantType2[nn])
                            & (as.character(FamilyID) == svfamid)
                            & (patID == 1 & RelationID == 2)))
                    dat2DadEqual <- subset(dat2,(((RefStartPos ==rf[nn]) & (RefEndPos==re[nn])) & 
                            ((perc_ref_query >= perc_similarity & perc_query_ref >= perc_similarity)) & (as.character(Type) == variantType2[nn])
            ...
            (length(grep("translocation", variantType2[nn])) >= 1)) {
                ...
                #dat2$perc <- as.numeric(dat2$Size)/size1
                    stt12 <- str_split(as.character(dat2$nanoID), pattern = "\\.")
                    famid <- c(); relnid <- c()
                    dat2$nanoID <- sapply(stt12, function(x) x[1])
                    dat2$RelationID <- sapply(stt12, function(x) x[2])
                    'dat2MomEqual <- subset(dat2,(((RefStartPos ==rf[nn]) & (RefEndPos==re[nn])) 
                            & (RefcontigID2 == chromo2[nn]) 
                            & ((perc_ref_query >= perc_similarity & perc_query_ref >= perc_similarity)) 
                            & (as.character(Type) == variantType2[nn])
                            & (as.character(nanoID) == svfamid)
                            & (patID == 1 & RelationID == 2)))
                    dat2DadEqual <- subset(dat2,(((RefStartPos ==rf[nn]) & (RefEndPos==re[nn]))           
                            & (RefcontigID2 == chromo2[nn]) 
                            & ((perc_ref_query >= perc_similarity & perc_query_ref >= perc_similarity)) 
                    ...
                    dat2DadRange <- subset(dat2,((((RefStartPos <= rf[nn] & 
                        ...
                        & (as.character(Type) == variantType2[nn])
                            & (as.character(nanoID) == svfamid)
                            & (patID == 1 & RelationID == 3)))'
                    dat2Frequencyinvtrans <- subset(dat2,(((as.character(Type) == "inversion" &
